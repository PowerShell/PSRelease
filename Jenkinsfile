	...
	stage('Build_Package_Publish') {
		...
		linux: {
			...
			node('Lnx-Slave-01') {			
				...
				sh "docker build --force-rm --tag microsoft/powershell:opensus13.2 /tmp/PSRelease/Images/microsoft_powershell_opensus13.2"			
				...
				sh "docker run --rm --volume /tmp/PSRelease:/mnt microsoft/powershell:opensus13.2 -c \"powershell -c \"/PowerShellPackage.ps1 -branch $BRANCH\" \""
